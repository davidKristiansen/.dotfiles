...
function M.switch()
    ...
    for _, wt in ipairs(worktrees) do
        ...
        items[#items + 1] = {
            branch = wt.branch, path = wt.path, exists = true,
                ...
                if not item.exists then
                    vim.ui.input({ prompt = "Worktree directory: ", default = sanitize_branch(item.branch) },
                        function(dir)
                            ...
                            vim.fn.system("git rev-parse --verify " .. vim.fn.shellescape(target) .. " 2>/dev/null")
...
function M.setup()
    local cmds = {
        switch = M.switch, s = M.switch,
        create = M.create, c = M.create,
        delete = M.delete, d = M.delete,
        prune  = M.prune,  p = M.prune,
    }
