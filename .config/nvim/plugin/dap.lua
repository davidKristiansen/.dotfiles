...
-- nvim-dap: debugging (Python + GTest).
-- Loaded on first keymap press (<F5> or <leader>d*).
...
local function load_dap()
  ...
  map('<leader>du',  '<cmd>DapViewToggle<cr>', 'Toggle View')
...
-- Stub keymaps: load DAP on first press, then replay the key
local stubs = {
  { '<F5>',       'DAP: Continue' },
...
for _, stub in ipairs(stubs) do
  ...
  vim.keymap.set('n', lhs, function()
    load_dap()
    -- Replay the keypress now that real keymaps are set
