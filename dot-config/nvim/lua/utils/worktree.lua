...
-- Git worktree workspace management module.
local M = {}
...
local function shell(cmd)
  return vim.fn.system(cmd), vim.v.shell_error == 0
end
...
local function unset_upstream(path, branch)
  shell(
    ('git -C %s branch --unset-upstream %s'):format(
...
local function get_worktrees()
  ...
  for line in output:gmatch('[^\r\n]+') do
    if line:match('^worktree ') then
      if
        current.path
        and not vim.fn.fnamemodify(current.path, ':t'):match('^%.')
        and vim.fn.isdirectory(current.path) == 1
      then
...
local function create_and_switch(cmd, path, msg)
  ...
  if ok then
    ...
    if switch_to_worktree(abs) then
      notify(msg .. abs)
    end
  else
...
function M.switch()
  ...
  if not branches or not worktrees then
    return notify('Git operation failed', vim.log.levels.ERROR)
  end
  ...
  for _, wt in ipairs(worktrees) do
    ...
    items[#items + 1] = {
      ...
      path = wt.path,
      exists = true,
      display = marker .. padded_branch .. '  ' .. rel,
    }
  end
  for _, b in ipairs(branches) do
    if not wt_map[b] then
      ...
      items[#items + 1] = {
        branch = b,
        exists = false,
        display = '          ' .. padded_branch .. '  (no worktree)',
      }
    end
  end
      ...
      actions = {
        ['default'] = function(sel)
          ...
          if not item.exists then
            vim.ui.input(
              { prompt = 'Worktree directory: ', default = sanitize_branch(item.branch) },
              function(dir)
                ...
                if not path then
                  return notify('Failed to determine worktree base', vim.log.levels.ERROR)
                end
                local target = item.branch
                vim.fn.system(
                  'git rev-parse --verify ' .. vim.fn.shellescape(target) .. ' 2>/dev/null'
                )
                if vim.v.shell_error ~= 0 then
                  target = 'origin/' .. item.branch
                end
                create_and_switch(
                  ('git worktree add %s %s'):format(
                    vim.fn.shellescape(path),
          ...
          elseif item.path ~= cwd then
            if switch_to_worktree(item.path) then
              notify('Switched to: ' .. item.path)
            end
          end
        end,
      },
...
function M.create()
  vim.ui.input({ prompt = 'Branch name: ' }, function(branch)
    ...
    vim.ui.input(
      { prompt = 'Worktree directory: ', default = sanitize_branch(branch) },
      function(dir)
        if not dir or dir == '' then
          return
        end
        ...
        if not branches then
          return notify('Failed to list branches', vim.log.levels.ERROR)
        end

        local default_br = vim.trim(
          vim.fn.system(
        ...
        table.sort(branches, function(a, b)
          ...
          if a == default_br then
            return true
          end
          if b == default_br then
            return false
          end
          local ar, br = a:match('^origin/'), b:match('^origin/')
          if ar and not br then
            return true
          end
          if br and not ar then
            return false
          end
          return a < b
        end)
              ...
              if ok then
                ...
                vim.defer_fn(function()
                  vim.ui.select({ 'Yes', 'No' }, { prompt = 'Switch to new worktree?' }, function(c)
                    if c == 'Yes' then
                      create_and_switch('true', path, 'Switched to: ')
                    end
                  end)
                end, 50)
              else
...
function M.delete()
  ...
  if #worktrees == 0 then
    return notify('No other worktrees to delete')
  end

  local max_w = 0
  for _, wt in ipairs(worktrees) do
    max_w = math.max(max_w, #wt.branch)
  end
      ...
      ['default'] = function(sel)
        ...
        vim.defer_fn(function()
          vim.ui.select({ 'Yes', 'No' }, { prompt = "Delete '" .. wt.branch .. "'?" }, function(c)
            ...
            if not ok then
              _, ok = shell('git worktree remove --force ' .. vim.fn.shellescape(wt.path))
            end
            if ok then
              ...
              vim.defer_fn(function()
                vim.ui.select(
                  { 'Yes', 'No' },
                  { prompt = "Delete branch '" .. wt.branch .. "'?" },
                  function(d)
                    if d == 'Yes' then
                      local out, bok = shell('git branch -D ' .. vim.fn.shellescape(wt.branch))
                      notify(
                        bok and ('Deleted branch: ' .. wt.branch) or ('Failed:\n' .. out),
...
function M.prune()
  vim.fn.system('git worktree prune')
  notify('Pruned stale worktree entries')
end
...
function M.setup()
  ...
  vim.api.nvim_create_user_command('Worktree', function(opts)
    ...
    if fn then
      ...
      notify('Usage: :Worktree {switch|create|delete|prune}', vim.log.levels.WARN)
    end
  end, {
    nargs = '+',
    complete = function(_, line)
      ...
      return #a == 2
          and vim.tbl_filter(function(c)
            return vim.startswith(c, a[2])
          end, { 'switch', 'create', 'delete', 'prune' })
        or {}
    end,
