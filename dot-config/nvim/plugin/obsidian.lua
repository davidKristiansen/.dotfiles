...
local vault_path = (os.getenv('XDG_DATA_HOME') or (os.getenv('HOME') .. '/.local/share'))
  .. '/vault'

local function realpath(p)
  local ok, r = pcall(vim.uv.fs_realpath, p)
...
require('utils.lazy').add({
  ...
  keys = {
    ...
    },
    {
      ...
      silent = true,
      desc = 'Obsidian: Yesterday (workday)',
    },
    {
      '<leader>nT',
      '<cmd>Obsidian tomorrow<cr>',
      silent = true,
      desc = 'Obsidian: Tomorrow (workday)',
    },
    {
      '<leader>nd',
      '<cmd>Obsidian dailies -2 1<cr>',
      silent = true,
      desc = 'Obsidian: Dailies (range)',
    },
    {
      '<leader>nM',
  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd('FileType', {
      ...
      callback = function(ev)
        if not is_in_vault(ev.buf) then
          return
        end
        ...
        bmap(
          ...
          'Obsidian: Extract selection -> note'
        )

        -- Smart checkbox toggle + auto-move for todo files
        local bufname = vim.api.nvim_buf_get_name(ev.buf)
        if bufname:match('todo%.md$') then
          bmap(ev.buf, 'n', '<leader>nc', function()
            require('utils.todo').cycle(1)
          end, 'Todo: Cycle checkbox forward')
          bmap(ev.buf, 'n', '<C-a>', function()
            require('utils.todo').cycle(1)
          end, 'Todo: Cycle checkbox forward')
          bmap(ev.buf, 'n', '<C-x>', function()
            require('utils.todo').cycle(-1)
          end, 'Todo: Cycle checkbox backward')
          bmap(ev.buf, 'n', '<leader>n1', function()
            require('utils.todo').set('[ ]')
          end, 'Todo: Set [ ] Inbox')
          bmap(ev.buf, 'n', '<leader>n2', function()
            require('utils.todo').set('[/]')
          end, 'Todo: Set [/] In Progress')
          bmap(ev.buf, 'n', '<leader>n3', function()
            require('utils.todo').set('[>]')
          end, 'Todo: Set [>] Waiting')
          bmap(ev.buf, 'n', '<leader>n4', function()
            require('utils.todo').set('[x]')
          end, 'Todo: Set [x] Done')
        else
          bmap(
            ev.buf,
